...
// ── Temas em alta ───────────────────────────────────────────────
async function temasEmAlta(area) {
  const prev = area === "previdenciario";
  if (BACKEND === "nenhum")
  ...
  const prompt = comBusca
    ? ("Hoje é " + diaSemana + ", " + dataHoje + ".\n\n" +
       "Você é o pauteiro de um escritório de advocacia em Brasília. " +
       (prev ? "FOCO: DIREITO PREVIDENCIÁRIO — INSS, benefícios, perícia, aposentadoria, revisões, BPC/LOAS. "
             : "FOCO: DIREITO DO TRABALHO — CLT, rescisão, jornada, assédio, verbas. ") + "\n\n" +
    ...
    : ("Hoje é " + diaSemana + ", " + dataHoje + ".\n\n" +
       "Você é o pauteiro de um escritório de advocacia em Brasília. " +
       (prev ? "FOCO: DIREITO PREVIDENCIÁRIO — INSS, benefícios, perícia, aposentadoria, BPC/LOAS. "
             : "FOCO: DIREITO DO TRABALHO — CLT, rescisão, jornada, assédio, verbas. ") + "\n\n" +
  ...
  const regras = prev
    ? "Regras: exatamente 8 temas, TODOS de direito previdenciário; relevância 'alta' ou 'media'; " +
      "área sempre 'previdenciario'; títulos formulados como TEMA de carrossel, em linguagem simples, " +
      "pensando em quem está doente, afastado ou teve o pedido negado."
    : "Regras: exatamente 8 temas; mínimo 5 trabalhista, mínimo 1 previdenciário, mínimo 1 cotidiano; " +
...
const server = http.createServer(async (req, res) => {
  ...
  if (caminho === "/api/trending-themes" && req.method === "GET") {
    let area = "trabalhista";
    try { area = new URL(req.url, "http://x").searchParams.get("area") || "trabalhista"; } catch (e) {}
    return enviarJSON(res, 200, await temasEmAlta(area));
  }
